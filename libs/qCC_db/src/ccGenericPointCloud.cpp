...
CCCoreLib::ReferenceCloud* ccGenericPointCloud::getTheHighlightedPoints(const VisibilityTableType* visTable/*=nullptr*/, bool silent/*=false*/) const
{
	...
	unsigned pointCount = 0;
	{
		for (unsigned i = 0; i < count; ++i)
		{
			if (visTable->at(i) == CCCoreLib::POINT_HIGHLIGHTED)
			{
				++pointCount;
			}
		}
