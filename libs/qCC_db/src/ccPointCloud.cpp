...
ccPointCloud::ccPointCloud(QString name/*=QString()*/, unsigned uniqueID/*=ccUniqueIDGenerator::InvalidUniqueID*/) throw()
	...
	, m_fileDir("")
	, m_trajectory(false)
	, m_trajectoryPtr(nullptr)
	, m_imagePointCloud(false)
{
...
void ccPointCloud::applyRigidTransformation(const ccGLMatrix& trans)
{
	...
	if (isTrajectory()) 
	{
		ccLog::Error("Point Cloud is trajectory cannot be transformed itself, need to transform only the pointcloud");
		return; }
	...
	//Take care of the trajectory 
	// x,y,z,qw,qx,qy,qz for the cameras
	if (m_trajectoryPtr)
	{
		for (int j = 0; j < m_trajectoryPtr->size(); j++)
		{
			// Get qx
			int qxField = m_trajectoryPtr->getScalarFieldIndexByName("qx");
			if (qxField == -1) { return; }
			m_trajectoryPtr->setCurrentScalarField(qxField);
			double qx = (double)m_trajectoryPtr->getPointScalarValue(j);

			// Get qy
			int qyField = m_trajectoryPtr->getScalarFieldIndexByName("qy");
			if (qyField == -1) { return; }
			m_trajectoryPtr->setCurrentScalarField(qyField);
			double qy = (double)m_trajectoryPtr->getPointScalarValue(j);

			// Get qz
			int qzField = m_trajectoryPtr->getScalarFieldIndexByName("qz");
			if (qzField == -1) { return; }
			m_trajectoryPtr->setCurrentScalarField(qzField);
			double qz = (double)m_trajectoryPtr->getPointScalarValue(j);

			// Get qw
			int qwField = m_trajectoryPtr->getScalarFieldIndexByName("qw");
			if (qwField == -1) { return; }
			m_trajectoryPtr->setCurrentScalarField(qwField);
			double qw = (double)m_trajectoryPtr->getPointScalarValue(j);

			// Get pt
			const CCVector3* pt = m_trajectoryPtr->getPoint(j);
			double x = pt->x;
			double y = pt->y;
			double z = pt->z;

			ccGLMatrix poseMat;
			CCCoreLib::SquareMatrix rotMat(3);
			double quaternion[4];
			quaternion[0] = qw;
			quaternion[1] = qx;
			quaternion[2] = qy;
			...
			poseMat.getTranslation()[0] = x;
			poseMat.getTranslation()[1] = y;
			poseMat.getTranslation()[2] = z;

			poseMat = trans * poseMat  ;


			CCCoreLib::SquareMatrixd sqrMat(poseMat.data(), true);

			double newQuaternion[4];
			sqrMat.toQuaternion(newQuaternion);
			m_trajectoryPtr->setCurrentScalarField(qwField);
			m_trajectoryPtr->setPointScalarValue(j, newQuaternion[0]);
			m_trajectoryPtr->setCurrentScalarField(qxField);
			m_trajectoryPtr->setPointScalarValue(j, newQuaternion[1]);
			m_trajectoryPtr->setCurrentScalarField(qyField);
			m_trajectoryPtr->setPointScalarValue(j, newQuaternion[2]);
			m_trajectoryPtr->setCurrentScalarField(qzField);
			m_trajectoryPtr->setPointScalarValue(j, newQuaternion[3]);
		}
		for (unsigned i = 0; i < m_trajectoryPtr->size(); i++)
		{
			trans.apply(*m_trajectoryPtr->point(i));
		}
		m_trajectoryPtr->applyGLTransformation_recursive();
		m_trajectoryPtr->prepareDisplayForRefresh_recursive();
	}
...
bool ccPointCloud::toFile_MeOnly(QFile& out) const
{
	...
	//name (dataVersion>=22)
	{
		...
		outStream << m_fileImageRelative;
	}
	{
		QDataStream outStream(&out);
		outStream << m_imagePrefix;
	}
	{
		QDataStream outStream(&out);
		outStream << m_imageSuffix;
	}
	{
		out.write((const char*)&m_trajectory, sizeof(bool));
	}
	{
		out.write((const char*)&m_imagePointCloud, sizeof(bool));
	}
	return true;
}
...
bool ccPointCloud::fromFile_MeOnly(QFile& in, short dataVersion, int flags, LoadedIDMap& oldToNewIDMap)
{
	...
	// Image Folders and suffix
	{
		QDataStream inStream(&in);
		inStream >> m_fileImageRelative;
	}
	{
		QDataStream inStream(&in);
		inStream >> m_imagePrefix;
	}
	{
		QDataStream inStream(&in);
		inStream >> m_imageSuffix;
	}
	if (in.read((char*)&m_trajectory, sizeof(bool)) < 0)
		return ReadError();
	if (in.read((char*)&m_imagePointCloud, sizeof(bool)) < 0)
		return ReadError();

	// Verify if the temp image folder corresponds to the same hardDrive
	QFileInfo file(in);
	QDir fileDir(file.absoluteDir());
	m_fileDir = file.absoluteDir().path();
	QString absolutePath = fileDir.cleanPath(fileDir.absoluteFilePath(m_fileImageRelative));
	if (absolutePath.size()>0)
	{
		m_imageFolder = absolutePath;
	}
	else 
	{
		m_imageFolder = m_fileDir;
	}
...
void ccPointCloud::setImageFolder(QString folder) {
	m_imageFolder = folder;

	if (m_fileDir.size() <= 0)
	{
		//If theres no existign file path (uploaded with other extension than .bin

		m_fileDir = folder;
	}
	QDir fileDir(m_fileDir);
	QDir infoDir(fileDir.relativeFilePath(m_imageFolder));
	m_fileImageRelative = infoDir.path();
};
...
std::vector<std::string> ccPointCloud::getScalarNameByCamId(int id)
{
	std::string x;
	std::string y;
	std::string z;
...
std::vector<double> ccPointCloud::getIntrinsicByCamId(int id)
{
