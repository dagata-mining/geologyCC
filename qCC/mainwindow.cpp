...
MainWindow::MainWindow()
	...
	, m_ccRoot(nullptr)
	, m_uiFrozen(false)
	, m_ccStichedImageViewer(nullptr)
	...
	, m_ppImg(false)
{
	...
	//image-viewer
	{
		m_ccStichedImageViewer = new ccStichedImageViewer(m_UI->DockableImageViewer, m_pickingHub);
		m_UI->DockableImageViewer->layout()->addWidget(m_ccStichedImageViewer);
	}


	tabifyDockWidget(m_UI->DockableImage, m_UI->DockableConsole);
	tabifyDockWidget(m_UI->DockableProperties, m_UI->DockableDBTree);
...
void MainWindow::connectActions()
{
	...
	connect(m_UI->actionSetViewCam,						&QAction::triggered, this, &MainWindow::toggleActiveWindowViewerBasedCam);
	...
	//Change pose position
	connect(m_UI->actionUpPose,						&QAction::triggered, this, &MainWindow::doActionUpPose);
	connect(m_UI->actionDownPose,						&QAction::triggered, this, &MainWindow::doActionDownPose);
			...
			else
			{
				...
				*firstCloud += pc;
				...
				if (firstCloud->size() == countBefore + countAdded)
				{
					...
					// Sorting by cam Id to have a sorted trajectory 
					int scalarIdIndex = firstCloud->getScalarFieldIndexByName("id");
					if (scalarIdIndex!= -1) 
					{
						firstCloud->setCurrentScalarField(scalarIdIndex);
						ccLog::Print(QString::number(firstCloud->size()));
						// Creating the sorting array
						ccLog::Print("Ordering the trajectory");
						std::vector<std::vector<int>> orderArray;
						for (int i = 0; i < (int)firstCloud->size(); ++i)
						{							
							orderArray.push_back({ i,(int)firstCloud->getPointScalarValue(i) });
							ccLog::Print(QString::number((int)firstCloud->getPointScalarValue(i)));
						}
						// Sort the array 
						std::sort(orderArray.begin(), orderArray.end(), sortcol);
						// Create a new point cloud with the right order
						CCCoreLib::ReferenceCloud* newCloud = new CCCoreLib::ReferenceCloud(firstCloud);
						for (int i = 0; i < (int)firstCloud->size(); ++i)
						{
							newCloud->addPointIndex(orderArray[i][0]);
						}
						ccPointCloud* trajectory = static_cast<ccPointCloud*>(firstCloud)->partialClone(newCloud);
						delete newCloud;
						ccPointCloud* firstCloudTemp = firstCloud;
						firstCloud = trajectory;
						delete firstCloudTemp;
...
void MainWindow::freezeUI(bool state)
{
	...
	//m_UI->DockableImageViewer->findChild<QWidget*>("stichedWidget")->setDisabled(state);
...
void MainWindow::activatePointPickingImg()
{
	ccGLWindow* win = getActiveGLWindow();
	if (!win)
	{
...
void MainWindow::deactivatePointPickingImg(bool state)
{
...
void MainWindow::toggleActiveWindowViewerBasedPerspective()
{
	...
	{
		const ccViewportParameters& params = win->getViewportParameters();
		if (params.perspectiveView && !params.objectCenteredView && !checkStereoMode(win) && params.cameraView ) //we need to check this only if we are already in viewer-based perspective mode
		{
			updateViewModePopUpMenu(win);
			return;
		}
...
void MainWindow::toggleActiveWindowViewerBasedCam()
{
...
void MainWindow::doActionUpPose()
{
	ccGetStichedImageViewer()->nodeUp();
}
void MainWindow::doActionDownPose()
{
	ccGetStichedImageViewer()->nodeDown();
}
