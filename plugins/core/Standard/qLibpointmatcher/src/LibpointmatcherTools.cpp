...
bool LibpointmatcherNormals::ComputeCorePointsNormals(CCCoreLib::GenericIndexedCloud* corePoints,
...
bool LibpointmatcherNormals::UpdateNormalOrientationsWithCloud(	CCCoreLib::GenericIndexedCloud* normCloud,
														NormsIndexesTableType& normsCodes,
														CCCoreLib::GenericIndexedCloud* orientationCloud,
														int maxThreadCount/*=0*/,
														CCCoreLib::GenericProgressCallback* progressCb/*=0*/)
{
	...
	{
		assert(false);
		ccLog::Warning("[LibpointmatcherTools::UpdateNormalOrientationsWithCloud] Cloud/normals set mismatch!");
		return false;
	}
...
void LibpointmatcherNormals::MakeNormalsHorizontal(NormsIndexesTableType& normsCodes)
{
	//for each normal
	unsigned count = normsCodes.currentSize();
	for (unsigned i = 0; i < count; i++)
...
void LibpointmatcherTools::ComputeStatistics(CCCoreLib::DgmOctree::NeighboursSet& set, bool useMedian, double& meanOrMedian, double& stdDevOrIQR)
{
...
bool LibpointmatcherTools::GuessBestParams(	ccPointCloud* cloud1,
									ccPointCloud* cloud2,
...
DP LibpointmatcherTools::ccToPointMatcher(ccPointCloud* cloud)
{
