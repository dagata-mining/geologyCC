...
void SaveImage(const ccImage* image, const QString& scanGUID, e57::ImageFile& imf, e57::VectorNode& images2D, std::vector<double> viewParams = {})
{
	assert(image);
	...
	if (viewParams.size() == 7)
	{
		e57::StructureNode pose = e57::StructureNode(imf);
		imageNode.set("pose", pose);

		e57::StructureNode rotation = e57::StructureNode(imf);
		rotation.set("w", e57::FloatNode(imf, viewParams[3]));
		rotation.set("x", e57::FloatNode(imf, viewParams[4]));
		rotation.set("y", e57::FloatNode(imf, viewParams[5]));
		rotation.set("z", e57::FloatNode(imf, viewParams[6]));
		pose.set("rotation", rotation);

		e57::StructureNode translation = e57::StructureNode(imf);
		translation.set("x", e57::FloatNode(imf, viewParams[0]));
		translation.set("y", e57::FloatNode(imf, viewParams[1]));
		translation.set("z", e57::FloatNode(imf, viewParams[2]));
		pose.set("translation", translation);
	}
...
CC_FILE_ERROR E57Filter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	ccPointCloud* pc;
	ccPointCloud* trajectory;
	bool lazType;

	if (entity->isA(CC_TYPES::POINT_CLOUD))
	{
		pc = static_cast<ccPointCloud*>(entity);
		trajectory = pc->getTrajectoryCloud();
		scans.push_back(pc);
		if (trajectory)
		{
			lazType = true;
			ccLog::Print("Cloud has trajectory linked. Exporting in Point Laz format");
		}
	}
	else
	{
		for (unsigned i=0; i<entity->getChildrenNumber(); ++i)
		{
			if (entity->getChild(i)->isA(CC_TYPES::POINT_CLOUD))
			{
				...
				pc = (static_cast<ccPointCloud*>(entity->getChild(i)));
				trajectory = pc->getTrajectoryCloud();
				scans.push_back(pc);
				if (trajectory)
				{
	...
	try
	{
		...
		/// Path name: "/creationDateTime
			...
			if (!SaveScan(pc, scanNode, imf, data3D, scanGUID, progressDlg.data()))
			{
			...
			if (result == CC_FERR_NO_ERROR)
			{
				...
				int imageCount = trajectory->size();
				...
				if (imageCount != 0)
				{
					...
					{
						progressDlg->setMethodTitle(QObject::tr("Write E57 file"));
						progressDlg->setInfo(QObject::tr("Point Laz Mining LiDAR Expert Cloud - Images: %1").arg(imageCount));
						progressDlg->start();
						QApplication::processEvents();
					}
					...
					for (unsigned j = 0; j < imageCount; ++j)
					{
						...
						int imageField = trajectory->getScalarFieldIndexByName("imageId");
						trajectory->setCurrentScalarField(imageField);
						int imageId = trajectory->getPointScalarValue(j);
						// Get qx
						int qxField = trajectory->getScalarFieldIndexByName("qx");
						if (qxField == -1) { break; }
						trajectory->setCurrentScalarField(qxField);
						double qx = (double)trajectory->getPointScalarValue(j);

						// Get qy
						int qyField = trajectory->getScalarFieldIndexByName("qy");
						if (qyField == -1) { break; }
						trajectory->setCurrentScalarField(qyField);
						double qy = (double)trajectory->getPointScalarValue(j);

						// Get qz
						int qzField = trajectory->getScalarFieldIndexByName("qz");
						if (qzField == -1) { break; }
						trajectory->setCurrentScalarField(qzField);
						double qz = (double)trajectory->getPointScalarValue(j);

						// Get qw
						int qwField = trajectory->getScalarFieldIndexByName("qw");
						if (qwField == -1) { break; }
						trajectory->setCurrentScalarField(qwField);
						double qw = (double)trajectory->getPointScalarValue(j);

						// Get pt
						const CCVector3* pt = trajectory->getPoint(j);
						double x = pt->x;
						double y = pt->y;
						double z = pt->z;

						std::vector<double> viewParams;
						viewParams = { x,y,z,qw,qx,qy,qz };
						ccHObject *imageObject = entity->find(imageId);
						ccImage *image = static_cast<ccImage*>(imageObject);
						assert(image->isKindOf(CC_TYPES::IMAGE));
						SaveImage(image, scanGUID, imf, images2D, viewParams);
						ccLog::Error("Here3");
...
static bool GetPoseInformation(const e57::StructureNode& node, ccGLMatrixd& poseMat, std::vector<double>& t)
{
	...
	if (node.isDefined("pose"))
	{
		...
		if (pose.isDefined("rotation"))
		{
			...
			t[3] = e57::FloatNode(rotNode.get("w")).value();
			t[4] = e57::FloatNode(rotNode.get("x")).value();
			t[5] = e57::FloatNode(rotNode.get("y")).value();
			t[6] = e57::FloatNode(rotNode.get("z")).value();
		...
		if (pose.isDefined("translation"))
		{
			...
			t[0] = e57::FloatNode(transNode.get("x")).value();
			t[1] = e57::FloatNode(transNode.get("y")).value();
			t[2] = e57::FloatNode(transNode.get("z")).value();
			validPoseMat = true;
		}
	}
	return validPoseMat;
...
static ccHObject* LoadScan(const e57::Node& node, QString& guidStr, ccProgressDialog* progressDlg = nullptr)
{
	...
	if (scanName == QString("Point Laz"))
	{
		ccLog::Print(QString("[Point Laz] Point Laz scan recognized, Shaft initialisation"));
		lazType = true;
	...
	ccGLMatrixd poseMat;
	std::vector<double> t;
	const bool validPoseMat = GetPoseInformation(scanNode, poseMat,t);
	...
	if (intensitySF)
	{
		...
		if (intensitySF->getMin() >= 0 && intensitySF->getMax() <= 1.0)
			...
			intensitySF->setColorScale(ccColorScalesManager::GetDefaultScale(ccColorScalesManager::GREY_SHAFT));
...
static ccHObject* LoadImage(const e57::Node& node, QString& associatedData3DGuid)
{
	...
	ccGLMatrixd poseMat;
	std::vector<double> t;
	bool validPoseMat = GetPoseInformation(imageNode, poseMat,t);
...
static std::pair<ccImage*, std::vector<double>> LoadImageLaz(const e57::Node& node, QString& associatedData3DGuid)
{
	std::pair<ccImage*, std::vector<double>> outputPair;
	ccGLMatrixd poseMat;
	std::vector<double> t{ 0,0,0,1,0,0,0 };

	if (node.type() != e57::E57_STRUCTURE)
	{
		ccLog::Warning("[E57Filter] Image nodes should be STRUCTURES!");
		outputPair = std::make_pair(nullptr, t);
		return outputPair;
	}
	e57::StructureNode imageNode(node);
	...
	if (validPoseMat)
	{
		outputPair = std::make_pair(imageObj, t);
	}
	else
	{
		outputPair = std::make_pair(nullptr, t);
	...
	try
	{
		...
		if (root.isDefined("/data3D"))
		{
			...
			for (unsigned i = 0; i < scanCount; ++i)
			{
				...
				scan = LoadScan(scanNode, scanGUID, showGlobalProgress ? nullptr : progressDlg.data());
				...
				if (scan)
				{
					if (scan->getName().isEmpty())
					{
						...
						if (scanNode.elementName() == "Point Laz")
						{
							lazType = true;
						}
			...
			for (unsigned i = 0; i < container.getChildrenNumber(); ++i)
			{
				if (container.getChild(i)->isA(CC_TYPES::POINT_CLOUD))
				{
					...
					if (sf)
					{
						sf->setSaturationStart(10);
						sf->setSaturationStop(61);
					}
				}
			}
		}
		...
		if (!s_cancelRequestedByUser && root.isDefined("/images2D"))
		{
			...
			if (imageCount)
			{
				...
				if (!lazType)
				{
					...
					cloudTrajectory = nullptr;
				}
				else 
				{
					cloudTrajectory = new ccPointCloud();
					cloudTrajectory->setTrajectory(true);
					std::vector<std::pair<ccHObject*, std::vector<double>>> posesImages;
					ccHObject* imagesFolder = new ccHObject("Images");
					container.addChild(imagesFolder);
					...
					for (unsigned i = 0; i < imageCount; ++i)
					{
						...
						QString associatedData3DGuid;
						//Laz Type
						std::pair<ccHObject*, std::vector<double>> pairTrajectory = LoadImageLaz(imageNode, associatedData3DGuid);
						ccHObject* image = pairTrajectory.first;
						std::vector<double> t = pairTrajectory.second;


						if (image)
						{				
							posesImages.push_back(pairTrajectory);
					...
					if (!cloudTrajectory->reserve(static_cast<unsigned>(posesImages.size())))
					{
						ccLog::Error("[E57] Not enough memory!");
						delete cloudTrajectory;
					}

					ccScalarField* qx = new ccScalarField("qx");
					ccScalarField* qy = new ccScalarField("qy");
					ccScalarField* qz = new ccScalarField("qz");
					ccScalarField* qw = new ccScalarField("qw");
					ccScalarField* imageId = new ccScalarField("imageId");
					...
					cloudTrajectory->addScalarField(qx);
					cloudTrajectory->addScalarField(qy);
					cloudTrajectory->addScalarField(qz);
					cloudTrajectory->addScalarField(qw);
					cloudTrajectory->addScalarField(imageId);
					cloudTrajectory->setName("trajectory");
					cloudTrajectory->setEnabled(false);

					container.addChild(cloudTrajectory);
					// Applies trajectory to the last scan
					ccPointCloud* scanCloud = dynamic_cast<ccPointCloud*>(scan);
					scanCloud->setImagePointCloud(true);
					scanCloud->setTrajectoryCloud(cloudTrajectory);
					scanCloud->initLOD();
				}
			}
		}
